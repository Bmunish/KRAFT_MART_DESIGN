...
function hydrateHeroCards(products) {
  document.querySelectorAll('.km-3d-card').forEach((card, index) => {
    ...
    const compare = Number(variant.compare_at_price);
    const price = Number(variant.price);
    card.innerHTML = `<a href="product-detail.html?handle=${encodeURIComponent(product.handle)}" class="km-3d-card-img-wrap">${image ? `<img src="${kmEscape(image)}" alt="${kmEscape(product.title)}">` : ''}</a><div class="km-3d-card-label"><span class="km-3d-card-tag">KraftMart collection</span><h3 class="km-3d-card-title">${kmEscape(product.title)}</h3><div class="km-3d-card-price">${kmMoney(price)}${compare > price ? `<span>${kmMoney(compare)}</span>` : ''}</div></div>`;
  });
}
...
function hydrateProductDetail(products) {
  ...
  detail.querySelector('[data-product-type]').textContent = product.product_type || product.vendor || 'KraftMart';
  ...
  if (add) add.textContent = variant.available ? `ADD TO CART — ${kmMoney(price)}` : 'SOLD OUT';
  ...
  if (thumbs) thumbs.innerHTML = images.slice(0, 5).map((item, index) => `<button class="km-pdp-thumb${index === 0 ? ' is-active' : ''}" type="button" aria-label="View image ${index + 1}"><img src="${kmEscape(item.src)}" alt="${kmEscape(item.alt || product.title)}"></button>`).join('');
  thumbs?.querySelectorAll('.km-pdp-thumb').forEach(button => button.addEventListener('click', () => { const selected = button.querySelector('img'); if (main && selected) { main.src = selected.src; main.alt = selected.alt; } thumbs.querySelectorAll('.km-pdp-thumb').forEach(item => item.classList.remove('is-active')); button.classList.add('is-active'); }));
}
...
async function initRealCatalogue() {
  try {
    ...
    document.querySelectorAll('[data-live-products]').forEach(grid => {
      const limit = grid.dataset.liveProducts === 'all' ? products.length : Number(grid.dataset.liveProducts);
      grid.innerHTML = products.slice(0, limit).map(kmProductCard).join('');
    });
    const count = document.getElementById('kmCatalogCount');
    if (count) count.textContent = `${products.length} KraftMart products`;
...
/**
 * NoiseBackground Interactive Hover Engine
 * Renders animated multi-color noise particles following cursor on cards
 */
function initNoiseBackground() {
  const cards = document.querySelectorAll('.km-noise-card, .km-product-card, .km-category-card');
  
  cards.forEach(card => {
    if (!card.classList.contains('km-noise-card')) {
      card.classList.add('km-noise-card');
    }
    
    // Check or append canvas
    let canvas = card.querySelector('.km-noise-canvas');
    if (!canvas) {
      canvas = document.createElement('canvas');
      canvas.className = 'km-noise-canvas';
      card.prepend(canvas);
    }
    ...
    function drawNoise() {
      ...
      // Color palette: Gold (197, 160, 89), Crimson (107, 29, 47), Ivory shimmer
      const grad1X = mouseX * w + Math.sin(time) * 30;
      const grad1Y = mouseY * h + Math.cos(time) * 30;
      
      const grad = ctx.createRadialGradient(grad1X, grad1Y, 10, grad1X, grad1Y, Math.max(w, h) * 0.8);
      grad.addColorStop(0, 'rgba(197, 160, 89, 0.45)');
      grad.addColorStop(0.5, 'rgba(107, 29, 47, 0.35)');
      grad.addColorStop(1, 'rgba(250, 247, 242, 0.15)');
...
/**
 * 3D Parallax Tilt Effect on Hero Visual Box
 */
...
/**
 * Bubble Cursor Sensitivity
 * Each bubble is gently pushed away from the cursor (magnetic repulsion).
 * Strength scales with proximity — the closer the cursor, the bigger the push.
 */
function initBubbleSensitivity() {
  ...
  let rafId = null;
  let mx = -9999, my = -9999; // off-screen default = no push
  ...
  document.addEventListener('mouseleave', () => {
    mx = -9999; my = -9999;
    bubbles.forEach(b => { b.style.setProperty('--bx', '0px'); b.style.setProperty('--by', '0px'); });
    rafId = null;
  });
